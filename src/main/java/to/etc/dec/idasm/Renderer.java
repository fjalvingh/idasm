...
final class Renderer implements IRunListener {
	...
	private enum Color {
		...
		Cyan(CYAN, CYAN_BACKGROUND),
		White(WHITE, WHITE_BACKGROUND),
		;
	...
	public Renderer fg(Color fg) {
		if(m_fg == fg)
			return this;
		m_fg = fg;
		m_sb.append(fg.getFg());
		return this;
	}

	public Renderer bg(Color bg) {
		if(m_bg == bg)
			return this;
		m_bg = bg;
		m_sb.append(bg.getBg());
		return this;
	}

	public Renderer inverse(boolean inverse) {
	...
	@Override public void renderRun(String text, short attribute) {
		if(attribute == 0) {
			inverse(false);
			fg(Color.White);
			bg(Color.Black);
			...
			boolean inverse = (attribute & F_INVERSE) != 0;
			inverse(inverse);
		}
		m_sb.append(text);
	}
